...
from app.models.catalog import SchoolRankingReference


@pytest.fixture
def ranking_catalog_file(seed_catalog):
    """注入 catalog 数据到测试数据库。"""
    seed_catalog(
        {
            "search_entry": {
...
def test_update_school_ranking_references(ranking_catalog_file, engine):
    with TestClient(app) as client:
    ...
    with Session(engine) as session:
        ...
        assert stored[0].label == "全国第 12 名"
