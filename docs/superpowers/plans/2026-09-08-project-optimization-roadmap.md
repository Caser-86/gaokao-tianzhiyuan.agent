...
- [ ] 注入限定大小的证据包，验证未知 citation、无来源数字、没有证据时的行为。
- [ ] 使用同一模型和同一问题对比“当前用户问题直接调用”与“上下文+证据+校验”；记录请求模型与返回模型，路由别名不能当固定模型版本。
- [ ] 两组共享预算、样本和评分；发布每条样本的优劣与失败原因，不保证增强方案必然获胜。

G2：多轮关键场景通过；所有展示引用能打开并与声明相符；对照报告可复现，缺数据场景明确。
...
- [ ] latest.json 记录验证日期、SHA、模式、报告路径与测试结果；文档链接到它或其对应报告，历史数字保留日期。
- [ ] 演示“目录证据 → 两轮追问 → 引用解释 → Provider故障降级 → trace/评测”。
- [ ] 覆盖首页、聊天、会话恢复、后台一次保存的浏览器 E2E；录制脱敏三分钟视频。
- [ ] 面试问答给出一个真实修复案例、一个质量失败样例、一次明确的技术取舍；说明离线指标的含义。
...
- [ ] 初始限制：消息4000字符、全局模型并发4、可配置单请求总时限30秒；输出 token 上限根据 Provider 实际支持验证。
- [ ] IP/主体组合速率限制与全局每日预算；匿名身份更换不得绕过全局预算；拒绝请求不调用模型。
- [ ] 配置 app logger，保存成功/失败后发最终 trace；保留未知 usage=null，记录请求模型、返回模型、耗时和降级原因。
- [ ] 注入超时、429、5xx；重试最多一次且受总时限/预算控制，不对所有失败重试。
- [ ] 验证真实服务启动日志有且只有预期 trace，不含原文、凭据。
